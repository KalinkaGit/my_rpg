...
void animation_player_menu(window_t *window, player_t *player)
{
    ...
    if (player->end_game == 1)
        player->end_timer ++;
    mob_update(window, player);
    sfClock_restart(window->clock);
}

void end_animation(window_t *window, player_t *player, menu_t *menu)
{
    printf("%d\n", player->end_timer);
    printf("POS : %f\n", sfSprite_getPosition(menu->end[2]).y);
    if (player->end_game == 1) {
        if (player->end_timer > 200 && player->end_timer < 500 && menu->color_fade.a != 255) {
            menu->color_fade.a += 1;
            sfRectangleShape_setFillColor(menu->fade, menu->color_fade);
        }
        if (player->end_timer > 500 && player->end_timer < 2600 && menu->color_fade.a != 0) {
            menu->color_fade.a -= 1;
            sfRectangleShape_setFillColor(menu->fade, menu->color_fade);
        }
        if (player->end_timer > 500) {
            window->is_in_menu = 1;
        }
        if (player->end_timer > 800 && sfSprite_getPosition(menu->end[0]).x >= 0)
            sfSprite_move(menu->end[0], (sfVector2f){-3, 0});
        if (player->end_timer > 1200 && sfSprite_getPosition(menu->end[1]).y >= 0)
            sfSprite_move(menu->end[1], (sfVector2f){0, -1});
        if (player->end_timer > 2600 && menu->color_fade.a != 255) {
            menu->color_fade.a += 1;
            sfRectangleShape_setFillColor(menu->fade, menu->color_fade);
        }
        if (player->end_timer > 2900 && player->end_timer < 3200 && sfSprite_getPosition(menu->end[2]).y >= 65)
            sfSprite_move(menu->end[2], (sfVector2f){0, -4});
        if (player->end_timer > 3400 && sfSprite_getPosition(menu->end[2]).y <= 300)
            sfSprite_move(menu->end[2], (sfVector2f){0, 4});
        if (player->end_timer > 3450 && player->end_timer < 3700 && sfSprite_getPosition(menu->end[3]).y >= 65)
            sfSprite_move(menu->end[3], (sfVector2f){0, -4});
        if (player->end_timer > 3520 && sfSprite_getPosition(menu->end[3]).y <= 300)
            sfSprite_move(menu->end[3], (sfVector2f){0, 4});
        if (player->end_timer > 3600)
            window->is_left = 1;
    }
}
